...
export default async function TermsPage({
  ...
  const sections = t.raw("legal.terms.sections") as { title: string; body: string }[];
        ...
        <Section>
          <article className="mx-auto max-w-3xl rounded-3xl border border-slate-700/60 bg-[#0b1120]/90 p-8 text-slate-200 shadow-[0_18px_40px_rgba(15,23,42,0.6)]">
            <header className="space-y-3 border-b border-slate-800/70 pb-6">
              <h1 className="text-3xl font-bold text-slate-50">{t("legal.terms.title")}</h1>
              <p className="text-sm text-slate-300">{t("legal.terms.subtitle")}</p>
            </header>
            ...
            <div className="mt-6 border-t border-slate-800/70 pt-4 text-sm text-slate-300">
              <div className="font-semibold text-slate-100">{t("legal.terms.contactTitle")}</div>
